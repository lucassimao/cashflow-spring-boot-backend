...
public interface GoalRepository extends PagingAndSortingRepository<Goal, Long>, TenantAwareRepository {
     ...
     @Query("SELECT g FROM Goal g WHERE :start is not null and :end is not null and CAST(g.start AS date) >= CAST(:start AS date) and CAST(g.end as date) <=  CAST(:end as date)")
